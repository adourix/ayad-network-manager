...
async function ensureSetUnlocked(name: string, type: string): Promise<void> {
  try { await execNft(["add", "set", TABLE_FAMILY, TABLE_NAME, name, "{", "type", type, ";", "}"]); }
  catch (error) { if (!isAlreadyExists(error)) throw error; }
}

async function ensureVpnEnforcementStateUnlocked(): Promise<void> {
  try { await execNft(["add", "table", TABLE_FAMILY, VPN_TABLE_NAME]); }
  catch (error) { if (!isAlreadyExists(error)) throw error; }

  try { await execNft(["add", "set", TABLE_FAMILY, VPN_TABLE_NAME, VPN_IP_SET_NAME, "{", "type", "ipv4_addr", ";", "}"]); }
  catch (error) { if (!isAlreadyExists(error)) throw error; }

  try {
    await execNft(["add", "chain", TABLE_FAMILY, VPN_TABLE_NAME, VPN_PREROUTING_CHAIN, "{", "type", "filter", "hook", "prerouting", "priority", "-301", ";", "policy", "accept", ";", "}"]);
  } catch (error) { if (!isAlreadyExists(error)) throw error; }
  ...
  if (!stdout.includes(VPN_IP_BLOCK_COMMENT)) {
    await execNft(["add", "rule", TABLE_FAMILY, VPN_TABLE_NAME, VPN_PREROUTING_CHAIN, "ip", "saddr", `@${VPN_IP_SET_NAME}`, "counter", "drop", "comment", VPN_IP_BLOCK_COMMENT]);
  }
}
...
async function ensureRuleAtPosition(comment: string, ruleArgs: string[], position: number, forceReconcile = false): Promise<void> {
  ...
  if (existing) await execNft(["delete", "rule", TABLE_FAMILY, TABLE_NAME, FORWARD_CHAIN, "handle", String(existing.handle)]);
  const remaining = existing ? rules.filter((rule) => rule.handle !== existing.handle) : rules;
  const target = remaining[position];
  if (target) await execNft(["insert", "rule", TABLE_FAMILY, TABLE_NAME, FORWARD_CHAIN, "position", String(target.handle), ...ruleArgs, "comment", comment]);
  else await execNft(["add", "rule", TABLE_FAMILY, TABLE_NAME, FORWARD_CHAIN, ...ruleArgs, "comment", comment]);
}
...
export async function getBlockedMacs(): Promise<Set<string>> {
  ...
  for (const value of match[1].split(",")) { const mac = value.trim().toLowerCase(); if (MAC_REGEX.test(mac)) blocked.add(mac); }
  return blocked;
}
export async function getBlockedIps(): Promise<Set<string>> {
  ...
  const match = stdout.match(/elements\s*=\s*\{([^}]*)\}/);
  if (!match?.[1]) return blocked;
  for (const value of match[1].split(",")) { const ip = value.trim(); if (IPV4_REGEX.test(ip)) blocked.add(ip); }
  return blocked;
}
