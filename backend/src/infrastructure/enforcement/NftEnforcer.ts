...
async function execNft(args: string[]): Promise<{ stdout: string; stderr: string }> {
  const mutating = args[0] === "add" || args[0] === "insert" || args[0] === "delete" || args[0] === "replace" || args[0] === "flush" || args[0] === "reset";
...
async function deleteElementUnlocked(setName: string, value: string): Promise<void> { const args = ["delete", "element", TABLE_FAMILY, TABLE_NAME, setName, `{ ${value} }`]; try { await execNft(args); } catch (error) { if (!isMissingElement(error)) throw error; } }
async function blockIpUnlocked(ip: string): Promise<void> { await ensureIpBlockRuleUnlocked(); await addElementUnlocked(IP_SET_NAME, validateIp(ip)); }
export async function blockIp(ip: string): Promise<void> { await withMutationLock(() => blockIpUnlocked(ip)); }
export async function unblockIp(ip: string): Promise<void> { await withMutationLock(() => deleteElementUnlocked(IP_SET_NAME, validateIp(ip))); }
export async function blockDevice(mac: string, ip?: string | null): Promise<void> { await withMutationLock(async () => { const validatedMac = validateMac(mac); await ensureMacBlockRuleUnlocked(); await addElementUnlocked(MAC_SET_NAME, validatedMac); if (ip) await blockIpUnlocked(ip); }); }
export async function unblockDevice(mac: string, ip?: string | null): Promise<void> { await withMutationLock(async () => { const validatedMac = validateMac(mac); await deleteElementUnlocked(MAC_SET_NAME, validatedMac); if (ip) await deleteElementUnlocked(IP_SET_NAME, validateIp(ip)); }); }
