...
export class SingleInterfaceVpnController implements VpnEnforcement {
  ...
  async configure(link: string): Promise<void> {
    ...
    const content = JSON.stringify(this.buildConfig(parsed));
  ...
  private async hasTunnelInterface(): Promise<boolean> {
    try {
      const result = await this.executor.execute("ip", ["-j", "link", "show"]);
      const links = JSON.parse(result.stdout) as Array<{ ifname?: unknown }>;
      return Array.isArray(links) && links.some((link) => link?.ifname === this.tunnelInterface);
    } catch {
      return false;
    }
  }
  ...
  private parseLink(link: string): VpnLink {
    ...
    if (/^vmess:\/\//i.test(value)) {
      return { protocol: "vmess", parsed: this.parseVmessLink(value) };
    }
    if (/^vless:\/\//i.test(value)) {
      return { protocol: "vless", parsed: this.parseVlessLink(value) };
    }
  ...
  private parseVmessLink(link: string): Record<string, unknown> {
    ...
    try {
      parsed = JSON.parse(Buffer.from(normalized, "base64").toString("utf8")) as Record<string, unknown>;
    } catch {
  ...
  private parseVlessLink(link: string): Record<string, unknown> {
    ...
    try {
      ...
      throw new Error("vless link is not a valid URL");
    }

    if (url.protocol.toLowerCase() !== "vless:") {
      throw new Error("invalid vless scheme");
    }
    ...
    if (!uuid || !server || !Number.isInteger(port) || port < 1 || port > 65535) {
      throw new Error("vless link is missing server, port, or id");
    }
    ...
    if (!["tcp", "ws", "grpc", "httpupgrade", "http"].includes(network)) {
    ...
    const realityPublicKey = query.pbk || "";
    const realityShortId = query.sid || "";

    if (security === "reality" && !realityPublicKey) {
      throw new Error("VLESS Reality link is missing pbk/public key");
    }

    if (security === "reality" && !realityShortId) {
      throw new Error("VLESS Reality link is missing sid/short id");
    }
  ...
  private buildConfig(link: VpnLink) {
    ...
    const dnsServers = config.network.dnsServers.map((server, index) => ({
      type: "udp",
      tag: `configured-dns-${index}`,
      server,
      server_port: 53,
    }));

    const finalDns = dnsServers[0]?.tag ?? "configured-dns-0";
    ...
    return {
      ...
      route: {
        ...
        rules: [{
          ip_cidr: config.network.dnsServers.map((server) => `${server}/32`),
          outbound: "direct",
        }],
  ...
  private buildVmessOutbound(parsed: Record<string, unknown>): Record<string, unknown> {
    ...
    if (!Number.isSafeInteger(alterId) || alterId < 0) {
    ...
    const outbound: Record<string, unknown> = {
      ...
      tls: tlsEnabled
        ? { enabled: true, ...(sni ? { server_name: sni } : {}) }
        : { enabled: false },
      domain_resolver: {
        server: "configured-dns-0",
        strategy: "ipv4_only",
      },
    };
  ...
  private buildVlessOutbound(parsed: Record<string, unknown>): Record<string, unknown> {
    ...
    const alpn = typeof parsed.alpn === "string" && parsed.alpn
      ? parsed.alpn.split(",").map((value) => value.trim()).filter(Boolean)
      : [];
    const fingerprint = typeof parsed.fp === "string" && parsed.fp ? parsed.fp : undefined;
    const security = String(parsed.security ?? "").toLowerCase();
    ...
    const outbound: Record<string, unknown> = {
      ...
      server_port: port,
      uuid,
      // Do not set network here. sing-box enables both TCP and UDP by default.
      // The URI's type=tcp describes the VLESS transport, not whether the
      // proxied destination may be TCP-only. Keeping network unset preserves UDP.
      packet_encoding: "xudp",
      domain_resolver: {
        server: "configured-dns-0",
        strategy: "ipv4_only",
    ...
    if (tlsEnabled) {
      ...
      if (security === "reality") {
        const publicKey = String(parsed.realityPublicKey ?? "");
        const shortId = String(parsed.realityShortId ?? "");

        if (!publicKey || !shortId) {
          throw new Error("VLESS Reality configuration is incomplete");
        }
  ...
  private async setNat(vpnConnected: boolean, vpnEnabled: boolean): Promise<void> {
    ...
    const nat = [
      ...
      "oifname",
      vpnConnected ? this.tunnelInterface : config.network.uplinkInterface,
      "masquerade",
      "comment",
      vpnConnected ? "ayad_nm_vpn_nat" : "ayad_nm_single_interface_nat",
    ];
  ...
  private async rules(
    ...
    const result = await this.executor.execute("nft", [
      "-j",
      "-a",
      "list",
      "chain",
