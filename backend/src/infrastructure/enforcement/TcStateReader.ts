...
export class LinuxTcStateReader implements TcStateReader {
  ...
  async getClassState(interfaceName: string, classId: string): Promise<TcClassState> {
    ...
    const target = new RegExp(`\\bclass\\s+htb\\s+1:${normalizedClassId}\\b`, "i");
    const line = result.stdout.split("\n").find((value) => target.test(value));
    if (line === undefined) return { exists: false, rate: null, ceil: null };
    return { exists: true, rate: line.match(/\brate\s+([^\s]+)/)?.[1] ?? null, ceil: line.match(/\bceil\s+([^\s]+)/)?.[1] ?? null };
  }
  ...
  async getDeviceClasses(interfaceName: string): Promise<TcDeviceClassState[]> {
    ...
    for (const line of result.stdout.split("\n")) {
      const match = line.match(/^\s*class\s+htb\s+1:([0-9a-f]+)\s+parent\s+1:1\b.*$/i);
      if (match === null || match[1] === undefined) continue;
      const classId = match[1].toLowerCase();
      if (classId === "1") continue;
      classes.push({ classId, rate: line.match(/\brate\s+([^\s]+)/)?.[1] ?? null, ceil: line.match(/\bceil\s+([^\s]+)/)?.[1] ?? null });
    }
    return classes;
  }
  ...
  async getDeviceFilters(interfaceName: string): Promise<TcDeviceFilterState[]> {
    ...
    for (let index = 0; index < lines.length; index += 1) {
      ...
      const filterBlock = lines.slice(currentFilterStart >= 0 ? currentFilterStart : index, index + 1).join(" ");
      if (currentIp === null) {
      ...
      filters.push({ classId, priority: currentPriority, ip: currentIp });
    }
    return filters;
  }
