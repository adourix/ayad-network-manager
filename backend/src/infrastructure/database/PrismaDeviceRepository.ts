...
function toDomain(row: {
  ...
  return {
    ...
    ip: row.ip ? IpAddress.create(row.ip) : null,
...
export class PrismaDeviceRepository implements DeviceRepository {
  private async assertNotProxyMac(mac: string, excludingDeviceId?: number): Promise<void> {
    const owner = await prisma.device.findFirst({
      where: {
        proxyMac: mac,
        ...(excludingDeviceId !== undefined ? { id: { not: excludingDeviceId } } : {}),
      },
      select: { id: true },
    });
  ...
  async findByMac(mac: MacAddress): Promise<Device | null> {
    const row = await prisma.device.findUnique({ where: { mac: mac.toString() } });
  ...
  async upsert(data: {
    ...
    const mac = data.mac.toString();
    const proxyMac = data.proxyMac?.toString() ?? null;
    await this.assertNotProxyMac(mac);
    if (proxyMac === mac) throw new Error(`Device MAC cannot also be its proxy MAC: ${mac}`);
    ...
    await this.assertNotProxyMac(reconciled.mac, existing?.id);
    if (reconciled.proxyMac === reconciled.mac) throw new Error(`Device MAC cannot also be its proxy MAC: ${reconciled.mac}`);
  ...
  async update(
    ...
    const nextProxyMac = data.proxyMac !== undefined ? data.proxyMac?.toString() ?? null : existing.proxyMac?.toString() ?? null;
    if (nextProxyMac === existing.mac.toString()) throw new Error(`Device MAC cannot also be its proxy MAC: ${existing.mac}`);

    const row = await prisma.device.update({
      where: { id },
      data: {
        ...(data.ip !== undefined && { ip: data.ip.toString() }),
        ...
        ...(data.proxyMac !== undefined && { proxyMac: nextProxyMac }),
