...
export class PrismaOperationsRepository implements OperationsRepository {
  async audit(data:{action:string;mac?:string|null;deviceId?:number|null;actor?:string|null;details?:unknown}){await prisma.auditLog.create({data:{action:data.action,mac:data.mac??null,deviceId:data.deviceId??null,actor:data.actor??null,details:data.details as any}});}
