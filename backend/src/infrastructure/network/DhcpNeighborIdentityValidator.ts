...
import type { DeviceIdentityValidator, ValidatedDeviceIdentity } from "./DeviceIdentityValidator.js";
...
function normalizeCapture(capture: CapturedIdentity): CapturedIdentity {
  return { ...capture, mac: normalizeMac(capture.mac), ...(capture.sourceIp ? { sourceIp: normalizeIp(capture.sourceIp) } : {}) };
}
...
class ObservationDebounce {
  ...
  observe(key: string, trusted: boolean, threshold = 3): boolean {
    if (!trusted) { this.counts.delete(key); return false; }
    const count = Math.min(threshold, (this.counts.get(key) ?? 0) + 1);
    this.counts.set(key, count);
    return count >= threshold;
  }
  clearExcept(keys: Set<string>): void {
    for (const key of this.counts.keys()) if (!keys.has(key)) this.counts.delete(key);
  }
}
...
function parseCidr(cidr: string): { network: number; mask: number } | null {
  const [ip, prefixString] = cidr.trim().split("/");
  if (!ip || !prefixString) return null;
  const prefix = Number(prefixString);
  if (!Number.isInteger(prefix) || prefix < 0 || prefix > 32) return null;
  const ipNumber = ipToNumber(ip);
  if (ipNumber === null) return null;
  const mask = prefix === 0 ? 0 : (0xffffffff << (32 - prefix)) >>> 0;
  return { network: (ipNumber & mask) >>> 0, mask };
}
function isIpInSubnet(ip: string, subnet: string): boolean {
  const ipNumber = ipToNumber(ip);
  const parsed = parseCidr(subnet);
  if (ipNumber === null || parsed === null) return false;
  return ((ipNumber & parsed.mask) >>> 0) === parsed.network;
}

export class DhcpNeighborIdentityValidator implements DeviceIdentityValidator {
  ...
  constructor(subnet: string, internalDebounce = true) {
    ...
    if (!subnet) throw new Error("Missing required environment variable: LAN_SUBNET");
    if (!parseCidr(subnet)) throw new Error(`Invalid LAN_SUBNET: ${subnet}`);
    this.lanSubnet = subnet;
  }
  ...
  validate(
    ...
    renewalObservedMacs: Set<string> = new Set(),
  ): ValidatedDeviceIdentity[] {
    const leaseByMac = new Map<string, DhcpLease>();
    const leaseByIp = new Map<string, DhcpLease>();
    const now = Math.floor(Date.now() / 1000);
    const normalizedCaptures = captures
      .map(normalizeCapture)
      .filter((capture) => MAC_REGEX.test(capture.mac))
      .filter((capture) => capture.sourceIp === undefined || (isValidIpv4(capture.sourceIp) && capture.sourceIp !== "0.0.0.0"));

    for (const lease of leases) {
      const mac = normalizeMac(lease.mac);
      const ip = normalizeIp(lease.ip);
      if (!MAC_REGEX.test(mac) || !isIpInSubnet(ip, this.lanSubnet)) continue;
      if (lease.expiry > 0 && lease.expiry < now) continue;
      const normalizedLease: DhcpLease = { ...lease, mac, ip };
      leaseByMac.set(mac, normalizedLease);
      leaseByIp.set(ip, normalizedLease);
    }
    ...
    for (const neighbor of neighbors) {
      ...
      if (lease && isUsableNeighborState(neighbor.state) && MAC_REGEX.test(neighborMac) && neighborMac !== lease.mac) {
    ...
    for (const neighbor of neighbors) {
      ...
      const neighborMac = normalizeMac(neighbor.mac);
      const neighborIp = normalizeIp(neighbor.ip);
      if (!MAC_REGEX.test(neighborMac) || !isIpInSubnet(neighborIp, this.lanSubnet)) continue;

      const lease = leaseByIp.get(neighborIp);
      if (lease) {
        const deviceMac = normalizeMac(lease.mac);
        if (validated.has(deviceMac)) continue;

        const neighborMatchesLease = neighborMac === deviceMac;
        const captureConfirmsLease = normalizedCaptures.some((capture) =>
          capture.mac === deviceMac && (capture.sourceIp === undefined || capture.sourceIp === neighborIp),
        );
        ...
        if (!neighborMatchesLease && !captureConfirmsLease) {
          ...
          const renewalObserved = renewalObservedMacs.has(deviceMac);
          ...
          validated.set(deviceMac, {
            ...
            identityValidated: mismatchDebounced && renewalObserved ? false : (captureConfirmsLease || neighborMatchesLease),
            identitySource: mismatchDebounced && renewalObserved ? "PROXY_UNCONFIRMED" : "DHCP",
            deferred: true,
          });
          continue;
        }
        ...
        validated.set(deviceMac, {
          ...
          identitySource: neighborMatchesLease ? "DHCP" : "DHCP_CONFIRMED_PROXY",
        });
        continue;
      }

      if (validated.has(neighborMac) || proxyMacs.has(neighborMac)) continue;
      const staticKey = `${neighborMac}|${neighborIp}`;
      observedKeys.add(staticKey);
      if (this.internalDebounce && !this.observationDebounce.observe(staticKey, true)) continue;
      validated.set(neighborMac, {
        mac: neighborMac,
        ip: neighborIp,
        hostname: null,
        clientId: null,
