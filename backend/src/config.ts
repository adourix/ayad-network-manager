...
function networkModeFromEnv(): 
  ...
  if (
    ...
    value !== "single-interface-ifb"
  ) {
    throw new Error(
      `Invalid NETWORK_MODE: ${value}. ` +
      `Expected "dual-interface" or "single-interface-ifb".`,
    );
  }
 ...
 *
 * UPLINK_INTERFACE:
 *   Interface facing the upstream router / ISP.
 *
 * We keep LAN_INTERFACE/WAN_INTERFACE as temporary
 * compatibility aliases so the existing code does not
 * break while we migrate the project to the new names
 * defined by the architecture specification.
 */
...
network: {
  ...
  clientGatewayIp:
    required("CLIENT_GATEWAY_IP"),

  clientSubnet:
    required("CLIENT_SUBNET"),
  ...
  quotaThrottleMbps:
    numberFromEnv("QUOTA_THROTTLE_MBPS", 0.5),

  vpnTunnelInterface:
    required("VPN_TUN_INTERFACE"),

  vpnConfigPath:
    required("SING_BOX_CONFIG_PATH"),

  vpnTunAddress:
    required("VPN_TUN_ADDRESS"),
