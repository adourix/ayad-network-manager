...
export class TrafficEnforcementService {
  ...
  async clearDownload(mac: string): Promise<void> {
    ...
    try {
      ...
      await this.operationsRepository?.audit({ action: "clear-download-limit", mac: device.mac.toString(), deviceId: device.id, details: { result: "failure", error: error instanceof Error ? error.message : String(error) } });
      throw error;
    }
  }
  ...
  async clearUpload(mac: string): Promise<void> {
    ...
    try {
      ...
      await this.operationsRepository?.audit({ action: "clear-upload-limit", mac: device.mac.toString(), deviceId: device.id, details: { result: "failure", error: error instanceof Error ? error.message : String(error) } });
      throw error;
    }
  }
    ...
    try {
      ...
      if (!policy) {
        ...
        if (policy.downloadLimit !== null) {
          this.trafficPolicyValidator.validate(device, { rateMbps: policy.downloadLimit });
          await this.trafficEnforcer.limitDownload(device, { rateMbps: policy.downloadLimit });
        } else {
          await this.trafficEnforcer.clearDownload(device);
        }
        if (policy.uploadLimit !== null) {
          this.trafficPolicyValidator.validate(device, { rateMbps: policy.uploadLimit });
          await this.trafficEnforcer.limitUpload(device, { rateMbps: policy.uploadLimit });
        } else {
          await this.trafficEnforcer.clearUpload(device);
        }
      }
      await this.operationsRepository?.audit({ action: "clear-quota-throttle", mac: device.mac.toString(), deviceId: device.id, details: { result: "success" } });
    } catch (error) {
