...
import type { QuotaPeriodData, QuotaPeriodRepository } from "../../domain/repositories/QuotaPeriodRepository.js";
...
export type QuotaAction = "notify-only" | "throttle" | "block";
...
function normalizeAction(value: string | null): QuotaAction | null {
  if (value === "notify-only" || value === "throttle" || value === "block") return value;
  return null;
}

function calculatePeriod(periodType: string, now: Date): { start: Date; end: Date } {
...
export class QuotaService {
  ...
  constructor(
    ...
    private readonly trafficEnforcementService: TrafficEnforcementService,
    private readonly firewallService: FirewallService,
    private readonly notificationRepository: NotificationRepository,
    private readonly quotaPeriodRepository: QuotaPeriodRepository,
  ) {}

  async getQuota(mac: string): Promise<QuotaView | null> {
    ...
    const period = await this.getOrCreateCurrentPeriod(device.id, policy.quotaPeriod, new Date());
    return this.buildView(policy.quota, policy.quotaPeriod, policy.quotaAction, period);
  }

  async recordUsage(mac: string, downloadBytes: bigint, uploadBytes: bigint, timestamp: Date = new Date()): Promise<QuotaView | null> {
    if (downloadBytes < 0n || uploadBytes < 0n) throw new Error("Quota usage cannot be negative");
    const device = await resolveDeviceIdentifier(this.deviceRepository, mac);
    if (!device) return null;
    const policy = await this.policyRepository.findByDeviceId(device.id);
  ...
  async resetQuota(mac: string): Promise<QuotaView | null> {
    ...
    if (!device) return null;
    const policy = await this.policyRepository.findByDeviceId(device.id);
    if (!policy || policy.quota === null || policy.quotaPeriod === null) throw new Error("Quota is not configured for this device");
    ...
    let period = await this.quotaPeriodRepository.findCurrent(device.id, policy.quotaPeriod, new Date());
    if (!period) {
      period = await this.quotaPeriodRepository.create({ deviceId: device.id, periodType: policy.quotaPeriod, periodStart: start, periodEnd: end });
    } else {
      period = await this.quotaPeriodRepository.reset(period.id, start, end);
    }
    await this.policyRepository.upsert(device.id, { quotaEnforcedAction: null });
    return this.buildView(policy.quota, policy.quotaPeriod, policy.quotaAction, period);
  }
  ...
  private async enforceQuotaAction(deviceId: number, mac: string): Promise<void> {
    ...
    if (!policy) return;
    const action = normalizeAction(policy.quotaAction);
    if (!action) {
      await this.clearQuotaEnforcement(deviceId, mac, policy.quotaEnforcedAction);
      return;
    }
    if (policy.quotaEnforcedAction === action) return;
    if (policy.quotaEnforcedAction) await this.clearQuotaEnforcement(deviceId, mac, policy.quotaEnforcedAction);
    switch (action) {
      case "notify-only":
        await this.notificationRepository.create({ deviceId, type: "quota-exhausted", message: `Device ${mac} has exhausted its traffic quota.` });
        break;
      case "throttle":
    ...
    await this.policyRepository.upsert(deviceId, { quotaEnforcedAction: action });
  }
  ...
  private async getOrCreateCurrentPeriod(deviceId: number, periodType: string, now: Date): Promise<QuotaPeriodData> {
    const { start, end } = calculatePeriod(periodType, now);
    let period = await this.quotaPeriodRepository.findCurrent(deviceId, periodType, now);
    if (!period) return this.quotaPeriodRepository.create({ deviceId, periodType, periodStart: start, periodEnd: end });
  ...
  private buildView(quota: bigint, quotaPeriod: string, quotaAction: string | null, period: QuotaPeriodData): QuotaView {
    const usedBytes = period.usedDownloadBytes + period.usedUploadBytes;
    const remainingBytes = quota > usedBytes ? quota - usedBytes : 0n;
    return {
      quota,
      quotaPeriod,
      quotaAction,
      ...
      usedDownloadBytes: period.usedDownloadBytes,
      usedUploadBytes: period.usedUploadBytes,
      usedBytes,
      remainingBytes,
      exhausted: usedBytes >= quota,
    };
  }
}
