...
export class SetupService {
  ...
  async apply(input: SetupApplyInput): Promise<SetupApplyResult> {
    ...
    if (uplinkRanges.length && !uplinkRanges.some((range) => networkOf(range) === networkOf(input.clientSubnet))) selectionErrors.push("client subnet must be the existing uplink subnet in Single-Interface + IFB mode");
    ...
    try {
      ...
      if (input.activate !== false) {
        ...
        await this.probe.run("enable", ["dnsmasq", "network-control-enforcement.service", "network-control-backend.service"]);
        await this.probe.run("restart", ["dnsmasq"]);
      }
      ...
      await this.rollback(snapshot); const message = error instanceof Error ? error.message : String(error);
      return { applied: false, configPath: this.paths.configPath, renderedFiles: [], health: { clientInterface: false, gatewayReachable: false, dhcpLeaseFile: false, outboundConnectivity: false, errors: [message] }, rolledBack: true, errors: [message] };
    }
  }

  async rollbackLatest(): Promise<void> {
    const snapshot = join(this.paths.snapshotDir, "latest");
    for (const [target, name] of [[this.paths.configPath, "config.env"], [this.paths.dnsmasqPath, "clients.conf"], [this.paths.nftablesPath, "network-control-system.nft"]] as const) { try { await fs.copyFile(join(snapshot, name), target); } catch {} }
  ...
  private async installPrerequisites(): Promise<void> {
    ...
    const update = await this.safe("apt-get", ["update"]); if (!update.ok) throw new Error(`failed to update apt package metadata: ${update.stderr || "apt-get update failed"}`);
    const install = await this.safe("apt-get", ["install", "-y", "--no-install-recommends", ...missing]); if (!install.ok) throw new Error(`failed to install prerequisites: ${install.stderr || "apt-get install failed"}`);
  }

  private async snapshot(): Promise<string> {
    const dir = join(this.paths.snapshotDir, `setup-${Date.now()}`); await fs.mkdir(dir, { recursive: true });
    for (const [target, name] of [[this.paths.configPath, "config.env"], [this.paths.dnsmasqPath, "clients.conf"], [this.paths.nftablesPath, "network-control-system.nft"]] as const) { try { await fs.copyFile(target, join(dir, name)); } catch {} }
    ...
    for (const [target, name] of [[this.paths.configPath, "config.env"], [this.paths.dnsmasqPath, "clients.conf"], [this.paths.nftablesPath, "network-control-system.nft"]] as const) { try { await fs.copyFile(target, join(latest, name)); } catch {} }
  ...
  private async health(client: string, gateway: string): Promise<SetupHealth> {
    const errors: string[] = [], link = await this.safe("ip", ["-j", "addr", "show", "dev", client]), clientInterface = link.ok && link.stdout.includes(client), gatewayReachable = (await this.safe("ping", ["-c", "1", "-W", "1", gateway])).ok, lease = await this.safe("cat", [this.paths.leasePath ?? "/var/lib/misc/dnsmasq.leases"]), dhcpLeaseFile = lease.ok && lease.stdout.trim().length > 0, outboundConnectivity = (await this.safe("ping", ["-c", "1", "-W", "2", "-I", client, "1.1.1.1"])).ok;
    if (!clientInterface) errors.push("client interface is not configured"); if (!gatewayReachable) errors.push("client gateway is not reachable"); if (!dhcpLeaseFile) errors.push("dnsmasq has not issued a lease"); if (!outboundConnectivity) errors.push("outbound connectivity test failed");
  ...
  private render(input: SetupApplyInput, gateway: string) {
    const [start, end] = dhcpRange(input.clientSubnet), vpnInterface = input.vpnTunnelInterface ?? process.env.VPN_TUN_INTERFACE ?? "tun0", vpnAddress = input.vpnTunAddress ?? process.env.VPN_TUN_ADDRESS ?? "172.19.0.1/30", singBoxPath = input.singBoxConfigPath ?? process.env.SING_BOX_CONFIG_PATH ?? "/etc/sing-box/config.json", reservationsPath = input.dhcpReservationsPath ?? process.env.DHCP_RESERVATIONS_PATH ?? "/var/lib/misc/network-control-reservations.conf";
    const config = [`CLIENT_INTERFACE=${input.clientInterface}`, `UPLINK_INTERFACE=${input.uplinkInterface}`, `CLIENT_SUBNET=${input.clientSubnet}`, `CLIENT_GATEWAY_IP=${gateway}`, `DHCP_RANGE_START=${start}`, `DHCP_RANGE_END=${end}`, `DNS_SERVERS=${input.dnsServers.join(",")}`, `DASHBOARD_PORT=${input.dashboardPort}`, `SSH_PORT=${input.sshPort}`, `UPLINK_BANDWIDTH_MBPS=${input.uplinkBandwidthMbps}`, "NETWORK_MODE=single-interface-ifb", `VPN_TUN_INTERFACE=${vpnInterface}`, `VPN_TUN_ADDRESS=${vpnAddress}`, `SING_BOX_CONFIG_PATH=${singBoxPath}`, `DHCP_RESERVATIONS_PATH=${reservationsPath}`].join("\n") + "\n";
    const dnsmasq = ["# Generated by network-control-system; do not edit.", "bind-interfaces", `interface=${input.clientInterface}`, "port=0", `dhcp-range=${start},${end},12h`, `dhcp-option=3,${gateway}`, `dhcp-option=6,${input.dnsServers.join(",")}`, `conf-file=${reservationsPath}`].join("\n") + "\n";
  ...
  private validateInput(input: SetupApplyInput) {
    ...
    const network = cidr.exec(input.clientSubnet); if (!network || prefix(input.clientSubnet) > 30 || network[1]!.split(".").some((part) => Number(part) > 255)) errors.push("clientSubnet must be a valid IPv4 network with prefix <= 30");
...
function networkOf(value: string): string | null {
  const match = cidr.exec(value); if (!match) return null;
  const octets = match[1]!.split(".").map(Number), bits = Number(match[2]), mask = bits === 0 ? 0 : (0xffffffff << (32 - bits)) >>> 0, address = ((octets[0]! << 24) | (octets[1]! << 16) | (octets[2]! << 8) | octets[3]!) >>> 0, network = address & mask;
...
function dhcpRange(subnet: string): [string, string] {
  const network = networkOf(subnet); if (!network) throw new Error("Invalid subnet");
  const base = network.split("/")[0].split(".").map(Number), bits = prefix(subnet);
  if (bits > 24) return [`${base[0]}.${base[1]}.${base[2]}.2`, `${base[0]}.${base[1]}.${base[2]}.254`];
  return [`${base[0]}.${base[1]}.${base[2]}.100`, `${base[0]}.${base[1]}.${base[2]}.200`];
}
