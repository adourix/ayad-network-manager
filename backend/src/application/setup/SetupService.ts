  ...
  async apply(input: SetupApplyInput): Promise<SetupApplyResult> {
    ...
    try {
      ...
      if (input.activate !== false) {
        ...
        await this.probe.run("systemctl", ["daemon-reload"]); await this.probe.run("systemctl", ["enable", "dnsmasq", "network-control-enforcement.service", "network-control-backend.service"]); await this.probe.run("systemctl", ["restart", "dnsmasq"]);
      }
...
function dhcpRange(subnet: string): [string, string] { const network = networkOf(subnet); if (!network) throw new Error("Invalid subnet"); const [a,b,c] = network.split("/")[0].split(".").map(Number); if (a === undefined || b === undefined || c === undefined) throw new Error("Invalid network address"); const bits = prefix(subnet); if (bits > 24) return [`${a}.${b}.${c}.2`, `${a}.${b}.${c}.254`]; return [`${a}.${b}.${c}.100`, `${a}.${b}.${c}.200`]; }