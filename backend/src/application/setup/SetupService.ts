...
export class SetupService {
  constructor(
    ...
    private readonly paths: SetupPaths = {
      ...
      leasePath: process.env.DHCP_LEASE_PATH ?? "/var/lib/misc/dnsmasq.leases",
    },
  ...
  async apply(input: SetupApplyInput): Promise<SetupApplyResult> {
    ...
    try {
      ...
      return { applied: true, configPath: this.paths.configPath, renderedFiles: [this.paths.configPath, this.paths.dnsmasqPath, this.paths.nftablesPath], health, rolledBack: false, errors: [] };
    } catch (error) {
  ...
  private async health(client: string, gateway: string): Promise<SetupHealth> {
    const errors: string[] = [];
    const link = await this.safe("ip", ["-j", "addr", "show", "dev", client]);
    const clientInterface = link.ok && link.stdout.includes(client);
    const gatewayReachable = (await this.safe("ping", ["-c", "1", "-W", "1", gateway])).ok;
    ...
    if (!gatewayReachable) errors.push("client gateway is not reachable");
    if (!dhcpLeaseFile) errors.push("dnsmasq has not issued a lease");
  ...
  private render(input: SetupApplyInput, gateway: string) {
    ...
    const reservations = input.dhcpReservationsPath ?? process.env.DHCP_RESERVATIONS_PATH ?? "/var/lib/misc/network-control-reservations.conf";
    const config = [`CLIENT_INTERFACE=${input.clientInterface}`, `UPLINK_INTERFACE=${input.uplinkInterface}`, `CLIENT_SUBNET=${input.clientSubnet}`, `CLIENT_GATEWAY_IP=${gateway}`, `DHCP_RANGE_START=${start}`, `DHCP_RANGE_END=${end}`, `DNS_SERVERS=${input.dnsServers.join(",")}`, `DASHBOARD_PORT=${input.dashboardPort}`, `SSH_PORT=${input.sshPort}`, `UPLINK_BANDWIDTH_MBPS=${input.uplinkBandwidthMbps}`, "NETWORK_MODE=single-interface-ifb", `VPN_TUN_INTERFACE=${vpnInterface}`, `VPN_TUN_ADDRESS=${vpnAddress}`, `SING_BOX_CONFIG_PATH=${singBoxPath}`, `DHCP_RESERVATIONS_PATH=${reservations}`].join("\n") + "\n";
    const dnsmasq = ["# Generated by network-control-system; do not edit.", "bind-interfaces", `interface=${input.clientInterface}`, "port=0", `dhcp-range=${start},${end},12h`, `dhcp-option=3,${gateway}`, `dhcp-option=6,${input.dnsServers.join(",")}`, `conf-file=${reservations}`].join("\n") + "\n";
    ...
    return { config, dnsmasq, nftables };
  }
...
function networkOf(value: string): string | null {
  ...
  const octets = match[1]!.split(".").map(Number); const bits = Number(match[2]); const mask = bits === 0 ? 0 : (0xffffffff << (32 - bits)) >>> 0;
...
function dhcpRange(subnet: string): [string, string] {
  ...
  const base = network.split("/")[0].split(".").map(Number); const bits = prefix(subnet);
