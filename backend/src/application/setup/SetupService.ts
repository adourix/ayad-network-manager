...
export class SetupService {
  constructor(
    ...
    private readonly paths: SetupPaths = {
      ...
      reservationsPath: process.env.DHCP_RESERVATIONS_PATH ?? RESERVATIONS_PATH,
      modulesLoadPath: process.env.IFB_MODULES_LOAD_PATH ?? MODULES_LOAD_PATH,
      dnsmasqOverridePath: process.env.DNSMASQ_SYSTEMD_OVERRIDE_PATH ?? DNSMASQ_OVERRIDE_PATH,
      legacyDnsmasqPath: process.env.LEGACY_DNSMASQ_PATH ?? LEGACY_DNSMASQ_PATH,
    },
  ...
  async apply(input: SetupApplyInput): Promise<SetupApplyResult> {
    ...
    try {
      ...
      if (input.activate !== false) {
        ...
        if (tables.ok && !/^table\s+ip\s+filter\s*$/m.test(tables.stdout)) {
          await this.probe.run("nft", ["-f", this.paths.nftablesPath]);
        } else if (!tables.ok) {
          throw new Error(`unable to inspect nftables tables: ${tables.stderr}`);
        }
      ...
      health = input.activate === false
        ? { clientInterface: true, gatewayReachable: true, dhcpLeaseFile: true, outboundConnectivity: true, errors: [] }
        : await this.health(input.clientInterface, gateway!);
  ...
  private async snapshot(): Promise<string> {
    ...
    for (const [target, name] of [
      ...
      [modulesLoadPath, "ifb-modules.conf"],
      [dnsmasqOverridePath, "dnsmasq-override.conf"],
      [legacyPath, "legacy-dns.conf"],
    ] as const) {
  ...
  private async restoreLegacyDnsmasqConfig(snapshot: string): Promise<void> {
    const legacyPath = this.paths.legacyDnsmasqPath ?? LEGACY_DNSMASQ_PATH;
    const disabledPath = `${legacyPath}.disabled`;
    try {
      await fs.copyFile(join(snapshot, "legacy-dns.conf"), legacyPath);
      try { await fs.unlink(disabledPath); } catch {}
    } catch {
      // The legacy configuration did not exist in the snapshot; leave it disabled if migration removed it.
    }
  }

  private async restartRestoredServices(): Promise<void> {
    try { await this.probe.run("systemctl", ["daemon-reload"]); } catch {}
    try { await this.probe.run("systemctl", ["restart", "network-control-enforcement.service"]); } catch {}
    try { await this.probe.run("systemctl", ["restart", "dnsmasq"]); } catch {}
  }
  ...
  private async health(client: string, gateway: string): Promise<SetupHealth> {
    ...
    const gatewayRoute = Boolean(gatewayRouteResult?.ok && new RegExp(`(?:^|\\s)dev\\s+${escapeRegExp(client)}(?:\\s|$)`).test(gatewayRouteResult.stdout));
  ...
  private render(input: SetupApplyInput, gateway: string) {
    ...
    const reservationsPath = input.dhcpReservationsPath ?? this.paths.reservationsPath ?? RESERVATIONS_PATH;
  ...
  private async migrateLegacyDnsmasqConfig(): Promise<void> {
    ...
    const disabledPath = `${legacyPath}.disabled`;
    try { await fs.rename(legacyPath, disabledPath); }
  ...
  private async writeAtomic(path: string | undefined, value: string) {
