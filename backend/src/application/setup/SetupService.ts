...
export class SetupService {
  ...
  async preflight(): Promise<SetupReport> {
    ...
    // dnsmasq is DHCP-only in this gateway setup, so DNS port 53 is not a
    // hard prerequisite. render() uses port=0 and advertises the configured
    // upstream DNS servers directly to DHCP clients.
  ...
  async apply(input: SetupApplyInput): Promise<SetupApplyResult> {
    ...
    try {
      ...
      if (input.activate !== false) {
        // Do not blindly load the complete generated nft file when the runtime
        // already owns the filter table. The normal enforcement startup is
        // idempotent and reconciles the live rules/sets safely.
      ...
      // The HTTP handler restarts the backend only after the setup response
      // has been flushed. Never restart the process from inside this request.
      return {
        applied: true,
  ...
  private async health(client: string, gateway: string): Promise<SetupHealth> {
    ...
    // The gateway IP belongs to this interface by design in Single-Interface
    // mode. Testing ping to the same host is not a meaningful reachability test,
    // so verify the address is configured and the route is usable instead.
    ...
    // An empty lease file is valid immediately after installation. The important
    // setup invariant is that dnsmasq can create/read its lease database; an
    // actual client lease may not exist until a client requests DHCP.
