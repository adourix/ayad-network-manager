...
export class SetupService {
  ...
  private async installPrerequisites(): Promise<void> {
    ...
    for (const pkg of PREREQUISITES) { const result = await this.safe("dpkg-query", ["-W", "-f=${Status}", pkg]); if (!result.ok || !/install ok installed/.test(result.stdout)) missing.push(pkg); }
  ...
  private async snapshot(includeNft: boolean): Promise<string> {
    ...
    const reservationsPath = this.paths.reservationsPath ?? RESERVATIONS_PATH;
    const modulesLoadPath = this.paths.modulesLoadPath ?? MODULES_LOAD_PATH;
    const dnsmasqOverridePath = this.paths.dnsmasqOverridePath ?? DNSMASQ_OVERRIDE_PATH;
    const legacyPath = this.paths.legacyDnsmasqPath ?? LEGACY_DNSMASQ_PATH;
  ...
  private async restoreSnapshotFiles(snapshot: string): Promise<void> {
    const reservationsPath = this.paths.reservationsPath ?? RESERVATIONS_PATH, modulesLoadPath = this.paths.modulesLoadPath ?? MODULES_LOAD_PATH, dnsmasqOverridePath = this.paths.dnsmasqOverridePath ?? DNSMASQ_OVERRIDE_PATH;
