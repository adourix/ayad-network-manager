...
export interface SetupPaths { configPath: string; dnsmasqPath: string; nftablesPath: string; snapshotDir: string; leasePath?: string; envPath?: string; }
...
export class SetupService {
  constructor(
    ...
    private readonly paths: SetupPaths = {
      ...
      envPath: process.env.ENV_FILE_PATH ?? join(process.cwd(), ".env"),
    },
  ...
  async apply(input: SetupApplyInput): Promise<SetupApplyResult> {
    ...
    if (uplinkRanges.length && !uplinkRanges.some((range) => networkOf(range) === networkOf(input.clientSubnet))) {
    ...
    try {
      ...
      const envPath = this.paths.envPath ?? join(process.cwd(), ".env");

      await this.writeAtomic(this.paths.configPath, rendered.config);
      await this.writeAtomic(envPath, await this.mergeEnv(envPath, rendered.envValues));
      ...
      if (input.activate !== false) {
        setTimeout(() => {
          void this.probe.run("systemctl", ["restart", "network-control-backend.service"]).catch(() => undefined);
        }, 250);
      }
      ...
      return {
        ...
        renderedFiles: [envPath, this.paths.configPath, this.paths.dnsmasqPath, this.paths.nftablesPath, rendered.reservationsPath],
        health,
        rolledBack: false,
        errors: [],
      };
  ...
  private render(input: SetupApplyInput, gateway: string) {
    ...
    const config = Object.entries(envValues).map(([key, value]) => `${key}=${value}`).join("\n") + "\n";
    const dnsmasq = [
    ...
    return { config, envValues, dnsmasq, nftables, reservationsPath };
  }

  private async mergeEnv(path: string, updates: Record<string, string>): Promise<string> {
    let existing = "";
    try { existing = await fs.readFile(path, "utf8"); } catch {}
    ...
    for (const [key, value] of Object.entries(updates)) if (!seen.has(key)) result.push(`${key}=${value}`);
