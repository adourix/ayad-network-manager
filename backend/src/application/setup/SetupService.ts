...
function dhcpRange(subnet: string): [string, string] { const network = networkOf(subnet); if (!network) throw new Error("Invalid subnet"); const base = network.split("/")[0].split(".").map(Number), bits = prefix(subnet); if (bits > 24) return [`${base[0]}.${base[1]}.${base[2]}.2`, `${base[0]}.${base[1]}.${base[2]}.254`]; return [`${base[0]}.${base[1]}.${base[2]}.100`, `${base[0]}.${base[1]}.${base[2]}.200`]; }
