...
const RESERVATIONS_PATH = "/var/lib/misc/network-control-reservations.conf";
const MODULES_LOAD_PATH = "/etc/modules-load.d/network-control-system.conf";
const DNSMASQ_OVERRIDE_PATH = "/etc/systemd/system/dnsmasq.service.d/network-control-system.conf";

export class SetupService {
  constructor(private readonly probe: SetupProbe, private readonly paths: SetupPaths = { configPath: process.env.SYSTEM_CONFIG_PATH ?? "/etc/network-control-system/config.env", dnsmasqPath: process.env.DNSMASQ_CONFIG_PATH ?? "/etc/dnsmasq.d/network-control-clients.conf", nftablesPath: process.env.NFTABLES_CONFIG_PATH ?? "/etc/nftables.d/network-control-system.nft", snapshotDir: process.env.SETUP_SNAPSHOT_DIR ?? "/var/lib/network-control/backups", leasePath: process.env.DHCP_LEASES_PATH ?? process.env.DHCP_LEASE_PATH ?? "/var/lib/misc/dnsmasq.leases", reservationsPath: process.env.DHCP_RESERVATIONS_PATH ?? RESERVATIONS_PATH, modulesLoadPath: process.env.IFB_MODULES_LOAD_PATH ?? MODULES_LOAD_PATH, dnsmasqOverridePath: process.env.DNSMASQ_SYSTEMD_OVERRIDE_PATH ?? DNSMASQ_OVERRIDE_PATH, legacyDnsmasqPath: process.env.LEGACY_DNSMASQ_PATH ?? LEGACY_DNSMASQ_PATH }, private readonly isRoot: () => boolean = () => typeof process.getuid !== "function" || process.getuid() === 0) {}
  ...
  async inspectNetwork(): Promise<SetupNetworkReport> {
    ...
    for (const row of Array.isArray(addresses) ? addresses : []) {
      ...
      addressMap.set(name, Array.isArray(row.addr_info) ? row.addr_info.filter((a: any) => a.family === "inet").map((a: any) => `${a.local}/${a.prefixlen}`) : []);
    }
  ...
  async apply(input: SetupApplyInput): Promise<SetupApplyResult> {
    ...
    const snapshot = await this.snapshot(input.activate !== false);
    let health: SetupHealth | null = null, gatewayAdded = false, servicesActivated = false;
    try {
      ...
      await this.writeAtomic(this.paths.modulesLoadPath ?? MODULES_LOAD_PATH, "ifb\n");
      await this.writeAtomic(this.paths.dnsmasqOverridePath ?? DNSMASQ_OVERRIDE_PATH, "[Service]\nRestart=on-failure\nRestartSec=5\n");
      if (input.activate !== false) {
        await this.migrateLegacyDnsmasqConfig();
        const tables = await this.safe("nft", ["list", "tables", "ip"]);
        if (tables.ok && !/^table\s+ip\s+filter\s*$/m.test(tables.stdout)) await this.probe.run("nft", ["-f", this.paths.nftablesPath]);
        else if (!tables.ok) throw new Error(`unable to inspect nftables tables: ${tables.stderr}`);
      ...
      return { applied: true, configPath: this.paths.configPath, renderedFiles: [this.paths.configPath, this.paths.dnsmasqPath, this.paths.nftablesPath, rendered.reservationsPath, this.paths.modulesLoadPath ?? MODULES_LOAD_PATH, this.paths.dnsmasqOverridePath ?? DNSMASQ_OVERRIDE_PATH], health, rolledBack: false, errors: [] };
    } catch (error) {
      if (gatewayAdded) await this.removeGatewayAddress(input.clientInterface, gateway!, input.clientSubnet);
      await this.rollback(snapshot, servicesActivated);
      const message = error instanceof Error ? error.message : String(error);
      return { applied: false, configPath: this.paths.configPath, renderedFiles: [], health: health ?? { clientInterface: false, gatewayReachable: false, dhcpLeaseFile: false, outboundConnectivity: false, errors: [message] }, rolledBack: true, errors: [message] };
    }
  }

  async restartBackend(): Promise<void> { await this.probe.run("systemctl", ["restart", "network-control-backend.service"]); }
  async rollbackLatest(): Promise<void> { const snapshot = join(this.paths.snapshotDir, "latest"); await this.restoreSnapshotFiles(snapshot); if (this.probe.restoreNft) try { await this.probe.restoreNft(join(snapshot, "nftables.bak")); } catch {} await this.restoreLegacyDnsmasqConfig(snapshot); await this.restartRestoredServices(); }
  ...
  private async snapshot(includeNft: boolean): Promise<string> {
    ...
    const reservationsPath = this.paths.reservationsPath ?? RESERVATIONS_PATH, modulesLoadPath = this.paths.modulesLoadPath ?? MODULES_LOAD_PATH, dnsmasqOverridePath = this.paths.dnsmasqOverridePath ?? DNSMASQ_OVERRIDE_PATH, legacyPath = this.paths.legacyDnsmasqPath ?? LEGACY_DNSMASQ_PATH;
    for (const [target, name] of [[this.paths.configPath, "config.env"], [this.paths.dnsmasqPath, "clients.conf"], [this.paths.nftablesPath, "network-control-system.nft"], [reservationsPath, "reservations.conf"], [modulesLoadPath, "ifb-modules.conf"], [dnsmasqOverridePath, "dnsmasq-override.conf"], [legacyPath, "legacy-dns.conf"]] as const) { try { await fs.copyFile(target, join(dir, name)); } catch {} }
    if (includeNft) { if (!this.probe.snapshotNft) throw new Error("nftables snapshot support is unavailable; setup cannot proceed safely"); const nft = await this.probe.snapshotNft(); if (!nft.trim()) throw new Error("nftables snapshot is empty; setup cannot proceed safely"); await fs.writeFile(join(dir, "nftables.bak"), nft, "utf8"); }
    const latest = join(this.paths.snapshotDir, "latest"); await fs.mkdir(latest, { recursive: true });
    for (const name of ["config.env", "clients.conf", "network-control-system.nft", "reservations.conf", "ifb-modules.conf", "dnsmasq-override.conf", "legacy-dns.conf", "nftables.bak"]) { try { await fs.copyFile(join(dir, name), join(latest, name)); } catch {} }
    return dir;
  }
  ...
  private async restoreSnapshotFiles(snapshot: string): Promise<void> { const reservationsPath = this.paths.reservationsPath ?? RESERVATIONS_PATH, modulesLoadPath = this.paths.modulesLoadPath ?? MODULES_LOAD_PATH, dnsmasqOverridePath = this.paths.dnsmasqOverridePath ?? DNSMASQ_OVERRIDE_PATH; for (const [target, name] of [[this.paths.configPath, "config.env"], [this.paths.dnsmasqPath, "clients.conf"], [this.paths.nftablesPath, "network-control-system.nft"], [reservationsPath, "reservations.conf"], [modulesLoadPath, "ifb-modules.conf"], [dnsmasqOverridePath, "dnsmasq-override.conf"]] as const) { try { await fs.copyFile(join(snapshot, name), target); } catch {} } }
  private async rollback(snapshot: string, restartServices: boolean): Promise<void> { await this.restoreSnapshotFiles(snapshot); if (this.probe.restoreNft) { try { await this.probe.restoreNft(join(snapshot, "nftables.bak")); } catch {} } await this.restoreLegacyDnsmasqConfig(snapshot); if (restartServices) await this.restartRestoredServices(); }
  private async restoreLegacyDnsmasqConfig(snapshot: string): Promise<void> { const legacyPath = this.paths.legacyDnsmasqPath ?? LEGACY_DNSMASQ_PATH, disabledPath = `${legacyPath}.disabled`; try { await fs.copyFile(join(snapshot, "legacy-dns.conf"), legacyPath); try { await fs.unlink(disabledPath); } catch {} } catch {} }
  private async restartRestoredServices(): Promise<void> { try { await this.probe.run("systemctl", ["daemon-reload"]); } catch {} try { await this.probe.run("systemctl", ["restart", "network-control-enforcement.service"]); } catch {} try { await this.probe.run("systemctl", ["restart", "dnsmasq"]); } catch {} }

  private async ensureGatewayAddress(client: string, gateway: string, subnet: string): Promise<boolean> { const prefixLength = prefix(subnet), normalized = gateway.trim().replace(/\/.*$/, ""); const current = await this.safe("ip", ["-j", "-4", "addr", "show", "dev", client]); if (!current.ok) throw new Error(`unable to inspect IPv4 addresses on ${client}: ${current.stderr}`); try { const parsed = JSON.parse(current.stdout) as unknown, rows = Array.isArray(parsed) ? parsed : [parsed], addresses = rows.flatMap((row: any) => Array.isArray(row.addr_info) ? row.addr_info.filter((a: any) => a.family === "inet").map((a: any) => String(a.local ?? "")) : []); if (addresses.includes(normalized)) return false; } catch { throw new Error(`unable to parse IPv4 addresses on ${client}`); } const added = await this.safe("ip", ["addr", "add", `${normalized}/${prefixLength}`, "dev", client]); if (!added.ok) throw new Error(`failed to configure client gateway ${normalized} on ${client}: ${added.stderr || "ip addr add failed"}`); return true; }
  private async removeGatewayAddress(client: string, gateway: string, subnet: string): Promise<void> { const normalized = gateway.trim().replace(/\/.*$/, ""); await this.safe("ip", ["addr", "del", `${normalized}/${prefix(subnet)}`, "dev", client]); }
  private async health(client: string, gateway: string): Promise<SetupHealth> { const errors: string[] = []; const link = await this.safe("ip", ["-j", "-4", "addr", "show", "dev", client]); let clientInterface = false, localAddresses: string[] = []; try { const parsed = JSON.parse(link.stdout) as unknown, rows = Array.isArray(parsed) ? parsed : [parsed]; for (const row of rows as Array<{ ifname?: string; addr_info?: Array<{ family?: string; local?: string }> }>) { if (row.ifname && row.ifname !== client) continue; if (!Array.isArray(row.addr_info)) continue; localAddresses.push(...row.addr_info.filter((addr) => addr.family === "inet" && typeof addr.local === "string").map((addr) => addr.local!.trim())); } localAddresses = [...new Set(localAddresses)]; clientInterface = link.ok && localAddresses.length > 0; } catch {} const normalizedGateway = gateway.trim().replace(/\/.*$/, ""); const gatewayConfigured = clientInterface && localAddresses.some((address) => address === normalizedGateway); const gatewayRouteResult = gatewayConfigured ? await this.safe("ip", ["route", "get", normalizedGateway]) : null; const gatewayRoute = Boolean(gatewayRouteResult?.ok && (new RegExp(`(?:^|\\s)dev\\s+${escapeRegExp(client)}(?:\\s|$)`).test(gatewayRouteResult.stdout) || gatewayRouteResult.stdout.includes(`\"dev\":\"${client}\"`))); const gatewayReachable = Boolean(gatewayConfigured && gatewayRoute); const lease = await this.safe("cat", [this.paths.leasePath ?? "/var/lib/misc/dnsmasq.leases"]); const dhcpLeaseFile = lease.ok && lease.stdout.trim().length > 0; const outboundConnectivity = (await this.safe("ping", ["-c", "1", "-W", "2", "-I", client, "1.1.1.1"])).ok; if (!clientInterface) errors.push(`client interface ${client} has no IPv4 address`); if (!gatewayConfigured) errors.push(`client gateway ${normalizedGateway} is not configured on ${client}`); else if (!gatewayRoute) errors.push(`client gateway ${normalizedGateway} has no local route on ${client}`); if (!dhcpLeaseFile) errors.push("dnsmasq lease file is empty or not readable; connect a client and retry setup health check"); if (!outboundConnectivity) errors.push("gateway outbound connectivity test failed"); return { clientInterface, gatewayReachable, dhcpLeaseFile, outboundConnectivity, errors }; }

  private render(input: SetupApplyInput, gateway: string) { const [start, end] = dhcpRange(input.clientSubnet); const vpnInterface = input.vpnTunnelInterface ?? process.env.VPN_TUN_INTERFACE ?? "tun0", vpnAddress = input.vpnTunAddress ?? process.env.VPN_TUN_ADDRESS ?? "172.19.0.1/30", singBoxPath = input.singBoxConfigPath ?? process.env.SING_BOX_CONFIG_PATH ?? "/etc/sing-box/config.json", reservationsPath = input.dhcpReservationsPath ?? this.paths.reservationsPath ?? RESERVATIONS_PATH, leasePath = this.paths.leasePath ?? "/var/lib/misc/dnsmasq.leases"; const values: Record<string, string> = { CLIENT_INTERFACE: input.clientInterface, UPLINK_INTERFACE: input.uplinkInterface, CLIENT_SUBNET: input.clientSubnet, CLIENT_GATEWAY_IP: gateway, DHCP_RANGE_START: start, DHCP_RANGE_END: end, DNS_SERVERS: input.dnsServers.join(","), DASHBOARD_PORT: String(input.dashboardPort), SSH_PORT: String(input.sshPort), UPLINK_BANDWIDTH_MBPS: String(input.uplinkBandwidthMbps), NETWORK_MODE: "single-interface-ifb", VPN_TUN_INTERFACE: vpnInterface, VPN_TUN_ADDRESS: vpnAddress, SING_BOX_CONFIG_PATH: singBoxPath, DHCP_RESERVATIONS_PATH: reservationsPath, DHCP_LEASES_PATH: leasePath, SETUP_COMPLETED: "true" }; const config = Object.entries(values).map(([key, value]) => `${key}=${value}`).join("\n") + "\n"; const dnsmasq = ["# Generated by network-control-system; do not edit.", "bind-interfaces", "port=0", `interface=${input.clientInterface}`, `dhcp-range=${start},${end},12h`, `dhcp-option=3,${gateway}`, `dhcp-option=6,${input.dnsServers.join(",")}`, `conf-file=${reservationsPath}`, ""].join("\n"); const nftables = ["# Generated by network-control-system; do not edit.", "table ip filter {", "  set blocked_macs { type ether_addr; }", "  set blocked_ips { type ipv4_addr; }", "  chain ayad_nm_input {", "    type filter hook input priority -300; policy accept;", `    tcp dport ${input.sshPort} accept comment \"ayad_nm_allow_ssh_management\"`, `    tcp dport ${input.dashboardPort} accept comment \"ayad_nm_allow_dashboard_management\"`, "  }", "  chain ayad_nm_forward {", "    type filter hook forward priority -300; policy accept;", "    ip saddr @blocked_ips drop comment \"ayad_nm_blocked_ips\"", "    ether saddr @blocked_macs drop comment \"ayad_nm_blocked_macs\"", "  }", "}", "table ip nat {", "  chain POSTROUTING {", "    type nat hook postrouting priority 100; policy accept;", `    ip saddr ${input.clientSubnet} oifname \"${input.uplinkInterface}\" masquerade comment \"ayad_nm_single_interface_nat\"`, "  }", "}", ""].join("\n"); return { config, dnsmasq, nftables, reservationsPath }; }
  private validateInput(input: SetupApplyInput) { const errors: string[] = []; if (!iface.test(input.clientInterface) || !iface.test(input.uplinkInterface)) errors.push("interface selections are invalid"); const network = cidr.exec(input.clientSubnet); if (!network || prefix(input.clientSubnet) > 30 || network[1]!.split(".").some((part) => Number(part) > 255)) errors.push("clientSubnet must be a valid IPv4 network with prefix <= 30"); if (input.clientGatewayIp !== undefined && (!ipv4.test(input.clientGatewayIp) || input.clientGatewayIp.split(".").some((part) => Number(part) > 255))) errors.push("CLIENT_GATEWAY_IP must be a valid IPv4 address"); if (input.vpnTunnelInterface && !iface.test(input.vpnTunnelInterface)) errors.push("VPN tunnel interface is invalid"); for (const value of [input.vpnTunAddress, input.singBoxConfigPath, input.dhcpReservationsPath]) if (value !== undefined && (!value.trim() || /[\r\n]/.test(value))) errors.push("setup path/address values must not be empty or contain newlines"); if (!Number.isInteger(input.uplinkBandwidthMbps) || input.uplinkBandwidthMbps <= 0) errors.push("uplink bandwidth must be a positive integer"); if (!Number.isInteger(input.dashboardPort) || input.dashboardPort < 1 || input.dashboardPort > 65535 || !Number.isInteger(input.sshPort) || input.sshPort < 1 || input.sshPort > 65535) errors.push("management ports must be valid TCP ports"); if (!Array.isArray(input.dnsServers) || input.dnsServers.length === 0 || input.dnsServers.some((x) => !ipv4.test(x) || x.split(".").some((part) => Number(part) > 255))) errors.push("DNS servers must be IPv4 addresses"); return errors; }
  private async migrateLegacyDnsmasqConfig(): Promise<void> { const legacyPath = this.paths.legacyDnsmasqPath ?? LEGACY_DNSMASQ_PATH; try { await fs.access(legacyPath); } catch { return; } const disabledPath = `${legacyPath}.disabled`; try { await fs.rename(legacyPath, disabledPath); } catch (error) { throw new Error(`failed to disable legacy dnsmasq configuration: ${error instanceof Error ? error.message : String(error)}`); } }
  ...
  private async writeAtomic(path: string | undefined, value: string) { if (!path) throw new Error("setup output path is not configured"); await fs.mkdir(dirname(path), { recursive: true }); const temp = `${path}.tmp-${process.pid}`; await fs.writeFile(temp, value, "utf8"); await fs.rename(temp, path); }
}
