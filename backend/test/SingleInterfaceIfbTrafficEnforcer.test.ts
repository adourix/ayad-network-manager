...
class FakeIfb implements IfbManager {
  ...
  async ensureUploadRedirect(_interfaceName: string, ip: string): Promise<void> { this.ensured = true; this.redirects.push(ip); }
  async removeUploadIp(_interfaceName: string, ip: string): Promise<void> { this.redirects = this.redirects.filter((value) => value !== ip); }
  async removeAllUploadRedirects(): Promise<void> { this.redirects = []; }
  async reconcileUploadRedirects(_interfaceName: string, expectedIps: Set<string>): Promise<void> { this.redirects = this.redirects.filter((value) => expectedIps.has(value)); }
  async remove(): Promise<void> { this.removed = true; this.ensured = false; this.redirects = []; }
}
...
class FakeReader implements TcStateReader {
  ...
  async getClassState(_i: string, classId: string): Promise<TcClassState> { const found = this.classes.find((value) => value.classId === classId.toLowerCase()); return found ? { exists: true, rate: found.rate, ceil: found.ceil } : { exists: false, rate: null, ceil: null }; }
  ...
  async getFilterState(_i: string, classId: string): Promise<TcFilterState> { const found = this.filters.find((value) => value.classId === classId.toLowerCase()); return found ? { exists: true, ip: found.ip, priority: found.priority } : { exists: false, ip: null, priority: null }; }
...
test("reconcile propagates state-reader failures", async () => { const executor = new FakeExecutor(); const failingReader: TcStateReader = { getClassState: async () => { throw new Error("tc unavailable"); }, getRootQdiscState: async () => ({ exists: false, kind: null }), getRootClassState: async () => ({ exists: false, rate: null, ceil: null }), getFilterState: async () => ({ exists: false, ip: null, priority: null }), getDeviceClasses: async () => { throw new Error("tc unavailable"); }, getDeviceFilters: async () => { throw new Error("tc unavailable"); } }; const enforcer = new SingleInterfaceIfbTrafficEnforcer(100n, "eno1", executor, new FakeIfb(), failingReader); await assert.rejects(() => enforcer.reconcileTrafficState(new Set()), /tc unavailable/); });
