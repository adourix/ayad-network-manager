...
test("schedule enforcement applies active values and restores base values outside the window", async () => {
  ...
  const catalog = {
    schedules: async () => [{
      ...
      name: "work",
      description: null,
      rules: [{ id: 1, dayOfWeek: 1, startTime: "09:00", endTime: "10:00", downloadLimit: 1n, uploadLimit: 1n, blocked: true }],
    }],
  };
  const traffic = {
    limitDownload: async (_m: string, v: any) => calls.push(`down:${v.rateMbps}`),
    limitUpload: async (_m: string, v: any) => calls.push(`up:${v.rateMbps}`),
    clearDownload: async () => calls.push("down:clear"),
    clearUpload: async () => calls.push("up:clear"),
  };
  const firewall = {
    block: async () => calls.push("block"),
    unblock: async () => calls.push("unblock"),
  };
  let current = new Date(2026, 7, 31, 9, 30);
  const service = new ScheduleEnforcementService(
    devices as any,
    policies as any,
    catalog as any,
    traffic as any,
  ...
  calls.length = 0;
  current = new Date(2026, 7, 31, 11, 0);
  await service.reconcile();
  assert.deepEqual(calls, ["unblock", "down:5", "up:2"]);
});
...
test("assigning a profile immediately inherits limits and quota", async () => {
  ...
  const repository = { upsert: async (_id: number, data: any) => { updates.push(data); return inherited; }, findByDeviceId: async () => inherited };
  ...
  const service = new DevicePolicyService({ findByMac: async () => device } as any, repository as any, catalog as any);
  const result = await service.upsertDevicePolicy(device.mac.toString(), { profileId: 9 });
  assert.deepEqual(updates, [{ profileId: 9, downloadLimit: 20n, uploadLimit: 5n, quota: 1000n, quotaPeriod: "monthly" }]);
  assert.equal(result.profileId, 9);
  assert.equal(result.downloadLimit, 20n);
  assert.equal(result.uploadLimit, 5n);
  assert.equal(result.quota, 1000n);
