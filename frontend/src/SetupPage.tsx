...
function Check({ ok }: { ok: boolean }) { return <span className={`setup-check ${ok ? "ok" : "bad"}`}>{ok ? "✓" : "!"}</span>; }
function subnetFromAddress(address: string): string {
  const match = address.match(/^(\d{1,3}(?:\.\d{1,3}){3})\/(\d{1,2})$/);
  if (!match) return "";
  const octets = match[1]!.split(".").map(Number), bits = Number(match[2]);
...
function SetupPage() {
  ...
  const apply = useMutation({
    mutationFn: () => setupApi.apply({ clientInterface, uplinkInterface, clientSubnet, uplinkBandwidthMbps: Number(bandwidth), dashboardPort: Number(dashboardPort), sshPort: Number(sshPort), dnsServers: dnsServers.split(",").map((value) => value.trim()).filter(Boolean), clientGatewayIp: gateway || undefined, activate: true }),
    onMutate: () => setError(""),
    onSuccess: (result) => {
  ...
  const selectInterface = (name: string) => {
    ...
    const subnet = network.data?.uplinkSubnets[name]?.[0] ?? subnetFromAddress(address);
  ...
  const subtitle = reconfigure ? "Review the current gateway configuration, change what you need, then apply the same validated setup path." : "Detect the gateway network, validate prerequisites, review the configuration, then apply the Single-Interface + IFB setup.";
  ...
  return (
    <div className="setup-page">
      ...
      <main className="setup-container">
        ...
        </div>{preflight.data?.errors.map((item) => <div className="setup-warning" key={item}>{item}</div>)}<div className="setup-actions"><button className="setup-primary" disabled={!preflightOk} onClick={() => setStep(1)}>Continue to network</button></div></section>}

        {step === 1 && <section className="setup-card"><div className="setup-card-head"><div><span className="setup-eyebrow">STEP 02</span><h2>Network interfaces</h2><p>For the current Single-Interface + IFB implementation, both roles use the same NIC.</p></div>{network.isFetching && <span className="setup-loading">Detecting…</span>}</div><div className="setup-interface-list">
          {interfaces.filter((item) => item.state === "UP").map((item) => <button key={item.name} type="button" className={`setup-interface ${uplinkInterface === item.name ? "selected" : ""}`} onClick={() => selectInterface(item.name)}><div><strong>{item.name}</strong><span>{item.kind ?? "network interface"}</span></div><div className="setup-interface-meta"><span>{item.addresses.join(", ") || "No IPv4 address"}</span><span>{item.mac ?? "No MAC"}</span></div></button>)}
        </div>{!interfaces.some((item) => item.state === "UP") && <div className="setup-warning">No active network interface was detected.</div>}{network.data?.errors.map((item) => <div className="setup-warning" key={item}>{item}</div>)}{diagnostics.data && <div className="setup-diagnostics"><span>Link speed <b>{diagnostics.data.linkSpeedMbps ? `${diagnostics.data.linkSpeedMbps} Mbps` : "Unknown"}</b></span><span>Duplex <b>{diagnostics.data.duplex ?? "Unknown"}</b></span>{diagnostics.data.warnings.map((item) => <span className="setup-warning inline" key={item}>{item}</span>)}</div>}<div className="setup-actions"><button className="setup-secondary" onClick={() => setStep(0)}>Back</button><button className="setup-primary" disabled={!networkOk || !uplinkInterface} onClick={() => setStep(2)}>Continue to configuration</button></div></section>}

        {step === 2 && <section className="setup-card"><div className="setup-card-head"><div><span className="setup-eyebrow">STEP 03</span><h2>Gateway configuration</h2><p>These values are loaded from the persisted runtime configuration when available.</p></div></div><div className="setup-form-grid">
          <label>Client interface<input value={clientInterface} readOnly/></label><label>Uplink interface<input value={uplinkInterface} readOnly/></label><label>Client subnet<select value={clientSubnet} onChange={(event) => setClientSubnet(event.target.value)}>{suggestedSubnets.map((subnet) => <option key={subnet} value={subnet}>{subnet}</option>)}{clientSubnet && !suggestedSubnets.includes(clientSubnet) && <option value={clientSubnet}>{clientSubnet}</option>}{detectedSubnet && !suggestedSubnets.includes(detectedSubnet) && detectedSubnet !== clientSubnet && <option value={detectedSubnet}>{detectedSubnet}</option>}</select></label><label>Gateway IP<input value={gateway} onChange={(event) => setGateway(event.target.value)} placeholder="Auto-detected"/></label><label>Uplink bandwidth (Mbps)<input type="number" min="1" step="1" value={bandwidth} onChange={(event) => setBandwidth(event.target.value)}/></label><label>Dashboard port<input type="number" min="1" max="65535" value={dashboardPort} onChange={(event) => setDashboardPort(event.target.value)}/></label><label>SSH port<input type="number" min="1" max="65535" value={sshPort} onChange={(event) => setSshPort(event.target.value)}/></label><label>DNS servers<input value={dnsServers} onChange={(event) => setDnsServers(event.target.value)} placeholder="1.1.1.1,8.8.8.8"/></label>
        </div><div className="setup-note"><b>Single-Interface + IFB</b><span>Download shaping uses the client destination IP; upload shaping uses IFB with the client source IP. DHCP uses port 0, so an existing DNS listener on port 53 does not block setup.</span></div>{error && <div className="setup-error">{error}</div>}<div className="setup-actions"><button className="setup-secondary" onClick={() => setStep(1)}>Back</button><button className="setup-primary" disabled={!formReady} onClick={() => { setError(""); setStep(3); }}>Review and apply</button></div></section>}

        {step === 3 && <section className="setup-card"><div className="setup-card-head"><div><span className="setup-eyebrow">STEP 04</span><h2>{applied ? "Setup applied" : "Apply configuration"}</h2><p>{applied ? "The gateway configuration has been applied and persisted." : "The backend will render the OS configuration, persist runtime values to /etc/network-control-system/config.env, activate services, and run the setup health checks."}</p></div>{apply.isPending && <span className="setup-loading">Applying…</span>}</div><div className="setup-form-grid"><label>Client interface<input value={clientInterface} readOnly/></label><label>Uplink interface<input value={uplinkInterface} readOnly/></label><label>Client subnet<input value={clientSubnet} readOnly/></label><label>Gateway IP<input value={gateway} readOnly/></label><label>Bandwidth<input value={`${bandwidth} Mbps`} readOnly/></label><label>Dashboard port<input value={dashboardPort} readOnly/></label><label>SSH port<input value={sshPort} readOnly/></label><label>DNS servers<input value={dnsServers} readOnly/></label></div>{apply.data && <div className="setup-check-grid"><div className="setup-check-row"><Check ok={apply.data.health.clientInterface}/><span>Client interface</span><small>{apply.data.health.clientInterface ? "Ready" : "Failed"}</small></div><div className="setup-check-row"><Check ok={apply.data.health.gatewayReachable}/><span>Gateway address and route</span><small>{apply.data.health.gatewayReachable ? "Ready" : "Failed"}</small></div><div className="setup-check-row"><Check ok={apply.data.health.dhcpLeaseFile}/><span>DHCP lease database</span><small>{apply.data.health.dhcpLeaseFile ? "Ready" : "Failed"}</small></div><div className="setup-check-row"><Check ok={apply.data.health.outboundConnectivity}/><span>Gateway outbound connectivity</span><small>{apply.data.health.outboundConnectivity ? "Ready" : "Failed"}</small></div></div>}{error && <div className="setup-error">{error}</div>}<div className="setup-actions">{!applied && <button className="setup-secondary" disabled={apply.isPending} onClick={() => setStep(2)}>Back</button>}{!applied && <button className="setup-primary" disabled={apply.isPending} onClick={() => apply.mutate()}>{apply.isPending ? "Applying…" : "Apply configuration"}</button>}{applied && <button className="setup-primary" onClick={finish}>{reconfigure ? "Return to dashboard" : "Continue to login"}</button>}</div></section>}
